...
{
    float ab, bc, ca, p = 0;
    // оскільки треба перебрати всі комбінації точок, складність алгоритму вийшла О(n^3)
...
{
    float dx2 = (p1.GetX() - p2.GetX())*(p1.GetX() - p2.GetX());
    float dy2 = (p1.GetY() - p2.GetY())*(p1.GetY() - p2.GetY());
    return sqrt(dx2+dy2);
}